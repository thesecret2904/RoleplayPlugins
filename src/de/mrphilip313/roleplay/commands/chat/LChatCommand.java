...
public class LChatCommand extends RoleplayCommand {
			...
			if (Players.getPlayerEntry(sender.getName()).isLoggedIn()) {
				if (args != null && args.length >= 1) {
					for (Player player : Bukkit.getServer().getOnlinePlayers()) {
						if(player.getLocation().distance(sender.getLocation()) <= ChatDistances.L_DISTANCE){
							player.sendMessage(ChatColor.GRAY + Players.getRPName(sender) + " sagt leise: " + Utils.buildString(args, 0));
						}
					}
				}
			} else {
